...
abilityObject.onAbilityCheck = function(player, target, ability)
    if player:hasStatusEffect(invaderXim.effect.PERPETUANCE) then
...
abilityObject.onUseAbility = function(player, target, ability)
    player:addStatusEffect(invaderXim.effect.PERPETUANCE, 1, 0, 60)

    return invaderXim.effect.PERPETUANCE
end
