...
abilityObject.onPetAbility = function(target, pet, petskill, summoner, action)
    local duration = 180 + summoner:getMod(invaderXim.mod.SUMMONING)
    ...
    if target:addStatusEffect(invaderXim.effect.SLOW, 3000, 0, duration) then
        petskill:setMsg(invaderXim.msg.basic.SKILL_ENFEEB_IS)
    else
        petskill:setMsg(invaderXim.msg.basic.SKILL_NO_EFFECT)
    end
