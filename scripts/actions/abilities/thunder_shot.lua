...
abilityObject.onAbilityCheck = function(player, target, ability)
    ...
    if
        player:hasItem(invaderXim.item.THUNDER_CARD, 0) or
...
abilityObject.onUseAbility = function(player, target, ability, action)
    ...
    dmg       = addBonusesAbility(player, invaderXim.element.THUNDER, target, dmg, params)
    ...
    dmg            = dmg * applyResistanceAbility(player, target, invaderXim.element.THUNDER, invaderXim.skill.NONE, bonusAcc)
    dmg            = dmg * invaderXim.spells.damage.calculateNukeAbsorbOrNullify(target, invaderXim.element.THUNDER)

    params.targetTPMult = 0 -- Quick Draw does not feed TP
    dmg                 = invaderXim.ability.takeDamage(target, player, params, true, dmg, invaderXim.attackType.MAGICAL, invaderXim.damageType.THUNDER, invaderXim.slot.RANGED, 1, 0, 0, 0, action, nil)

    if dmg > 0 then
        local effects = {}

        local shock = target:getStatusEffect(invaderXim.effect.SHOCK)
        ...
        if threnody ~= nil and threnody:getSubPower() == invaderXim.mod.WATER_MEVA then
    ...
    local _ = player:delItem(invaderXim.item.THUNDER_CARD, 1) or player:delItem(invaderXim.item.TRUMP_CARD, 1)
