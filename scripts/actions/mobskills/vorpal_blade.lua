...
mobskillObject.onMobSkillCheck = function(target, mob, skill)
    ...
    elseif mob:getFamily() == 176 then
        ...
        if mob:getAnimationSub() == 0 and mob:getMainJob() == invaderXim.job.BLU then
            return 0
        else
            return 1
        end
    elseif mob:getPool() ~= 4249 then
        mob:messageBasic(invaderXim.msg.basic.READIES_WS, 0, 40)
    end
...
mobskillObject.onMobWeaponSkill = function(target, mob, skill)
    if mob:getPool() == 4249 then -- Volker@Throne_Room only
        target:showText(mob, zones[invaderXim.zone.THRONE_ROOM].text.BLADE_ANSWER)
    end
