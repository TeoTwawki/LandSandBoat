...
spellObject.onSpellCast = function(caster, target, spell)
    ...
    local dmg = 10 + (1.035 * caster:getSkillLevel(invaderXim.skill.DARK_MAGIC))
    local targetHP = target:getHP()

    if dmg > (caster:getSkillLevel(invaderXim.skill.DARK_MAGIC) + 20) then
        dmg = (caster:getSkillLevel(invaderXim.skill.DARK_MAGIC) + 20)
    end
