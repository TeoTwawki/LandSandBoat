...
spellObject.onMagicCastingCheck = function(caster, target, spell)
    ...
    if caster:isMob() then
        return 0
    elseif caster:hasStatusEffect(invaderXim.effect.ELEMENTAL_SEAL) then
        return 0
    else
        return invaderXim.msg.basic.STATUS_PREVENTS
    end
end
...
spellObject.onSpellCast = function(caster, target, spell)
    ...
    local dmg = 0
    if caster:isPC() then
        dmg = ((100 + caster:getMod(invaderXim.mod.MATT)) / (100 + target:getMod(invaderXim.mod.MDEF))) * (caster:getStat(invaderXim.mod.INT) + caster:getSkillLevel(invaderXim.skill.ELEMENTAL_MAGIC) / 6) * 3.5
    ...
    then
        ...
        dmg = ((100 + caster:getMod(invaderXim.mod.MATT)) / (100 + target:getMod(invaderXim.mod.MDEF))) * (caster:getStat(invaderXim.mod.INT) + (caster:getMaxSkillLevel(caster:getMainLvl(), invaderXim.job.BLM, invaderXim.skill.ELEMENTAL_MAGIC)) / 6) * 9.4
    end
