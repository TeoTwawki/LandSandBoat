...
spellObject.onSpellCast = function(caster, target, spell)
    ...
    if resist >= 0.5 then
        target:addStatusEffect(invaderXim.effect.SLOW, 2000, 0, duration * resist)
        target:addStatusEffect(invaderXim.effect.SILENCE, 1, 0, duration * resist)
        target:addStatusEffect(invaderXim.effect.PARALYSIS, 15, 0, duration * resist)
        target:addStatusEffect(invaderXim.effect.BIND, 1, 0, duration * resist)
        target:addStatusEffect(invaderXim.effect.WEIGHT, 20, 0, duration * resist)
        target:addStatusEffect(invaderXim.effect.POISON, 4, 0, duration * resist)
        target:addStatusEffect(invaderXim.effect.BLINDNESS, 20, 0, duration * resist)
    end
