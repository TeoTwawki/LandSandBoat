...
spellObject.onSpellCast = function(caster, target, spell)
    ...
    if  bio == nil then -- if no bio, add dia dot
        target:addStatusEffect(invaderXim.effect.DIA, 2 + dotBonus, 3, duration, 0, 15, 2)
    elseif
        bio:getSubPower() == 10 or
        (invaderXim.settings.main.BIO_OVERWRITE == 1 and bio:getSubPower() <= 15) -- also erase same tier bio if BIO_OVERWRITE option is on (non-default)
