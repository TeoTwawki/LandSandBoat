...
invaderXim.combat.tp = invaderXim.combat.tp or {}
-----------------------------------

-- returns a single melee hit's TP return
invaderXim.combat.tp.getSingleMeleeHitTPReturn = function(actor, target, isZanshin)
    isZanshin = isZanshin or false -- optional input, defaults to false.

    local delay        = actor:getBaseDelay()
    local attackOutput = invaderXim.combat.tp.getModifiedDelayAndCanZanshin(actor, delay)
    local tpReturn     = invaderXim.combat.tp.calculateTPReturn(actor, attackOutput.modifiedDelay)

    if isZanshin and attackOutput.canZanshin then
        tpReturn = tpReturn + actor:getMerit(invaderXim.merit.IKISHOTEN) -- https://www.bg-wiki.com/ffxi/Ikishoten
    end

    local storeTPModifier = (100 + actor:getMod(invaderXim.mod.STORETP)) / 100

    return math.floor(tpReturn * storeTPModifier)
end

-- returns a PC weapon slot's TP return for a single hit
invaderXim.combat.tp.getSingleWeaponTPReturn = function(actor, slot)
...
invaderXim.combat.tp.getModifiedDelayAndCanZanshin = function(actor, delay)
    ...
    if actor:isDualWielding() then -- NOTE: this 'isDualWielding' may trip on non-PCs even if they are 'using h2h'. If this is rectified in core in the future this should fall through correctly.
        modifiedDelay = (delay * (100 - actor:getMod(invaderXim.mod.DUAL_WIELD)) / 100) / 2
    elseif actor:isUsingH2H() then
        if actor:getObjType() == invaderXim.objType.PC then            -- handle h2h with > 1 swing only on PC
            if
                actor:getEquippedItem(invaderXim.slot.SUB) ~= nil or   -- equipped shield = one swing
                actor:getSkillRank(invaderXim.skill.HAND_TO_HAND) == 0 -- zero h2h rank skill = one swing
            then
                modifiedDelay = math.max((delay - actor:getMod(invaderXim.mod.MARTIAL_ARTS)), 96) -- min delay of 96 total, https://www.bg-wiki.com/ffxi/Attack_Speed
                canZanshin    = true -- Zanshin can proc on an 'unarmed' swing               -- https://www.bg-wiki.com/ffxi/Zanshin
            else
                modifiedDelay = math.max((delay - actor:getMod(invaderXim.mod.MARTIAL_ARTS)) / 2, 48) -- min delay of 96 total so 96/2 per fist, https://www.bg-wiki.com/ffxi/Attack_Speed
            end
        else
            ...
            modifiedDelay = math.max((delay - actor:getMod(invaderXim.mod.MARTIAL_ARTS)) / 2, 48)
        end
    else -- single melee swing, either 1H or 2H
        canZanshin = true -- https://www.bg-wiki.com/ffxi/Zanshin
    end

    modifiedDelay = modifiedDelay * math.max((100 + actor:getMod(invaderXim.mod.DELAYP)) / 100, 0.85) -- minimum cap of -15% https://www.bg-wiki.com/ffxi/Attack_Speed. Undocumented if 15% + Claymore Grip goes above 15%.

    return ({ canZanshin = canZanshin , modifiedDelay = math.floor(modifiedDelay) })
end

-- returns a single melee hit's TP return
invaderXim.combat.tp.getSingleRangedHitTPReturn = function(actor, target)
    ...
    if delay > 0 then
        ...
        return math.floor(invaderXim.combat.tp.calculateTPReturn(actor, delay) * storeTPModifier)
    end
...
invaderXim.combat.tp.calculateTPReturn = function(gainee, delay)
    local tpReturn = 0

    if gainee and gainee:getObjType() ~= invaderXim.objType.MOB then -- Pets and PCs have been observed to use this formula
        if delay > 900 then
...
invaderXim.combat.tp.calculateTPGainOnPhysicalDamage = function(totalDamage, delay, actor, target)
    ...
    if totalDamage > 0 and target and actor then
        local attackOutput       = invaderXim.combat.tp.getModifiedDelayAndCanZanshin(actor, delay)
        local baseTPGain         = invaderXim.combat.tp.calculateTPReturn(target, attackOutput.modifiedDelay)
        local dAGI               = actor:getMod(invaderXim.mod.AGI) - target:getMod(invaderXim.mod.AGI)
        local inhibitTPModifier  = (100 - target:getMod(invaderXim.mod.INHIBIT_TP)) / 100                    -- no known cap: https://www.bg-wiki.com/ffxi/Monster_TP_gain#Inhibit_TP
        ...
        local subtleBlowMerits   = actor:getMerit(invaderXim.merit.SUBTLE_BLOW_EFFECT)
        local subtleBlowI        = math.min(actor:getMod(invaderXim.mod.SUBTLE_BLOW) + subtleBlowMerits, 50) -- cap of 50% https://www.bg-wiki.com/ffxi/Subtle_Blow
        local subtleBlowII       = actor:getMod(invaderXim.mod.SUBTLE_BLOW_II)                               -- no known cap
        ...
        local storeTPModifier    = (100 + target:getMod(invaderXim.mod.STORETP)) / 100
        ...
        if
            target:getObjType() == invaderXim.objType.MOB and
            actor:getObjType() ~= invaderXim.objType.MOB
        then
...
invaderXim.combat.tp.calculateTPGainOnMagicalDamage = function(totalDamage, actor, target)
    -- TODO: does dAGI penalty work against/for Trusts/Pets? Nothing is documented for this. Currently assuming mob only.
    if totalDamage > 0 and target and actor then
        local dAGI               = actor:getMod(invaderXim.mod.AGI) - target:getMod(invaderXim.mod.AGI)
        local inhibitTPModifier  = (100 - target:getMod(invaderXim.mod.INHIBIT_TP)) / 100                    -- no known cap: https://www.bg-wiki.com/ffxi/Monster_TP_gain#Inhibit_TP
