...
invaderXim.einherjar.makeLamp = function(player, chamberId, startTime, endTime)
    player:addItem({
        id = invaderXim.item.GLOWING_LAMP,
        exdata =
        {
            [ 0] = 0x1D + chamberId,
            [ 2] = 0x01,
...
invaderXim.einherjar.voidAllLamps = function(player, chamberId)
    for _, item in ipairs(player:findItems(invaderXim.item.GLOWING_LAMP)) do
        local lampData = invaderXim.einherjar.decypherLamp(item)
        if lampData.chamber == chamberId then
            invaderXim.einherjar.voidLamp(player, item)
        end
    end
end
...
invaderXim.einherjar.voidLamp = function(player, lampObj)
    player:delItemAt(invaderXim.item.GLOWING_LAMP, 1, lampObj:getLocationID(), lampObj:getSlotID())
    player:addItem({
        id = invaderXim.item.GLOWING_LAMP,
        exdata =
        {
...
invaderXim.einherjar.decypherLamp = function(lampObj)
...
invaderXim.einherjar.isLampExpired = function(lampObj)
...
invaderXim.einherjar.getMatchingLamps = function(player, chamberId, startTime)
    local matchingLamps = { }
...
invaderXim.einherjar.onLampCheck = function(player, lampObj)
    if not lampObj then
        return invaderXim.msg.basic.ITEM_UNABLE_TO_USE
    end

    if player:getZone():getID() ~= invaderXim.zone.HAZHALM_TESTING_GROUNDS then
        return invaderXim.msg.basic.ITEM_UNABLE_TO_USE
    end

    if invaderXim.einherjar.isLampExpired(lampObj) then
        invaderXim.einherjar.voidLamp(player, lampObj)
        return invaderXim.msg.basic.ITEM_UNABLE_TO_USE
    end

    if player:getFreeSlotsCount() <= 1 then -- <= 1 is not a typo -- see onLampUse
        return invaderXim.msg.basic.ITEM_NO_USE_INVENTORY
    end
...
invaderXim.einherjar.onLampUse = function(player, lampObj)
    if not lampObj then
        return
    end
    ...
    local chamberInstance = invaderXim.einherjar.getChamber(lampData.chamber)
    ...
    for _ = 1, 2 do
        invaderXim.einherjar.makeLamp(player, lampData.chamber, lampData.startTime, lampData.endTime)
    end
end

-- Player is dropping a Glowing Lamp
-- If they're in a chamber and have no matching lamps, ensure they're kicked out
invaderXim.einherjar.onLampDrop = function(player, lampObj)
    if player:getZone():getID() ~= invaderXim.zone.HAZHALM_TESTING_GROUNDS then
        return
    end
    ...
    if #invaderXim.einherjar.getMatchingLamps(player, lampData.chamber, lampData.startTime) == 0 then
