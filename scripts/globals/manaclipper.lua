...
-----------------------------------
xi = xi or {}
invaderXim.manaclipper = invaderXim.manaclipper or {}
...
invaderXim.manaclipper.location =
{
    SUNSET_DOCKS    = 0,
    PURGONORGO_ISLE = 1,
    MANACLIPPER     = 2,
...
{
    [invaderXim.manaclipper.location.SUNSET_DOCKS] =
    {
    ...
    [invaderXim.manaclipper.location.PURGONORGO_ISLE] =
    {
    ...
    [invaderXim.manaclipper.location.MANACLIPPER] =
    {
...
invaderXim.manaclipper.timekeeperOnTrigger = function(player, location, eventId)
    ...
    if schedule then
        ...
        if location == invaderXim.manaclipper.location.MANACLIPPER then
        ...
        printf('[warning] bad location %i in invaderXim.manaclipper.timekeeperOnTrigger', location)
    end
end

invaderXim.manaclipper.aboard = function(player, triggerArea, isAboard)
    player:setCharVar('[manaclipper]aboard', isAboard and triggerArea or 0)
end

invaderXim.manaclipper.onZoneIn = function(player)
    local zoneId = player:getZoneID()

    -- zoning onto manaclipper. set [manaclipper]arrivalEventId based on schedule.
    if zoneId == invaderXim.zone.MANACLIPPER then
        local schedule = manaclipperSchedule[invaderXim.manaclipper.location.MANACLIPPER]
        local currentTime = VanadielHour() * 60 + VanadielMinute()
        local nextEvent = nil
    ...
    elseif zoneId == invaderXim.zone.BIBIKI_BAY then
...
invaderXim.manaclipper.onTransportEvent = function(player, transport)
    ...
    if aboard == 1 then
        if player:hasKeyItem(invaderXim.ki.MANACLIPPER_TICKET) then
            player:delKeyItem(invaderXim.ki.MANACLIPPER_TICKET)
            player:startEvent(14)
        elseif player:hasKeyItem(invaderXim.ki.MANACLIPPER_MULTI_TICKET) then
            local uses = player:getCharVar('Manaclipper_Ticket')

            if uses == 1 then
                player:messageSpecial(ID.text.END_BILLET, 0, invaderXim.ki.MANACLIPPER_MULTI_TICKET)
                player:delKeyItem(invaderXim.ki.MANACLIPPER_MULTI_TICKET)
            else
                player:messageSpecial(ID.text.LEFT_BILLET, 0, invaderXim.ki.MANACLIPPER_MULTI_TICKET, uses - 1)
            end

            player:setCharVar('Manaclipper_Ticket', uses - 1)
            player:startEvent(14)
        else
            player:messageSpecial(ID.text.NO_BILLET, invaderXim.ki.MANACLIPPER_TICKET)
