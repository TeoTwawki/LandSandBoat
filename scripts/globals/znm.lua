...
invaderXim.znm.UpdateSanrakusMobs = function()
    SetServerVariable('[ZNM][Sanraku]Interest', math.random(#invaderXim.znm.SANRAKUS_INTEREST))
    SetServerVariable('[ZNM][Sanraku]Fauna', math.random(#invaderXim.znm.SANRAKUS_FAUNA))
    SetServerVariable('[ZNM][Sanraku]Trades', math.random(0, 250))
end

invaderXim.znm.serverPlateTrades = function()
    local currentTrades = GetServerVariable('[ZNM][Sanraku]Trades')

    if currentTrades >= 500 then
        invaderXim.znm.UpdateSanrakusMobs()
    else
...
invaderXim.znm.getSanrakusInterest = function()
    ...
    if interest == nil or interest == 0 then
        interest = math.random(#invaderXim.znm.SANRAKUS_INTEREST)
...
invaderXim.znm.getSanrakusFauna = function()
    local fauna = GetServerVariable('[ZNM][Sanraku]Fauna')

    -- Initialize the server var if it hasn't been already
    if fauna == nil or fauna == 0 then
        fauna = math.random(#invaderXim.znm.SANRAKUS_FAUNA)
...
invaderXim.znm.isCurrentFauna = function(plateData)
    -- local zeni     = plateData.zeni

    local data     = plateData.interestData
    local zoneID   = getHighBits16(data)
    local mobName  = plateData.name
    local faunaRow = invaderXim.znm.SANRAKUS_FAUNA[invaderXim.znm.getSanrakusFauna()]
...
invaderXim.znm.isCurrentSuperFamily = function(plateData)
    local data            = plateData.interestData
    local superFamily     = getLowBits16(data)
    local currectInterest = invaderXim.znm.getSanrakusInterest()
    local interestRow     = invaderXim.znm.SANRAKUS_INTEREST[currectInterest]
...
invaderXim.znm.isCurrentEcosystem = function(plateData)
    local data            = plateData.interestData
    local superFamily     = getLowBits16(data)
    local currectInterest = invaderXim.znm.getSanrakusInterest()
    local interestRow     = invaderXim.znm.SANRAKUS_INTEREST[currectInterest]
...
invaderXim.znm.calculatePlateZeni = function(player, plateData)
    -- Cache the soulplate value on the player
    local zeni  = plateData.zeni
    local bonus = 'none'

    if invaderXim.znm.isCurrentFauna(plateData) then
        zeni = zeni + invaderXim.znm.SOULPLATE_FAUNA
        bonus = 'Fauna'
    elseif invaderXim.znm.isCurrentSuperFamily(plateData) then
        zeni = zeni + invaderXim.znm.SOULPLATE_INTEREST
        bonus = 'superFamily'
    elseif invaderXim.znm.isCurrentEcosystem(plateData) then
        zeni = zeni + invaderXim.znm.SOULPLATE_ECOSYSTEM
        bonus = 'ecoSystem'
    end
    ...
    zeni = utils.clamp(zeni, invaderXim.znm.SOULPLATE_MIN_VALUE, invaderXim.znm.SOULPLATE_MAX_VALUE)
...
invaderXim.znm.soultrapper = invaderXim.znm.soultrapper or {}

-----------------------------------
-- onItemCheck
-----------------------------------

invaderXim.znm.soultrapper.onItemCheck = function(target, item, player, caster)
    -- can not be used on non mobs or Structure type mobs
    if
        not target:isMob() or
        (target:getFamily() >= 236 and target:getFamily() <= 239)
    then
        return invaderXim.msg.basic.ITEM_CANNOT_USE_TARGET
    end

    if
        player:hasStatusEffect(invaderXim.effect.INVISIBLE) or
        player:hasStatusEffect(invaderXim.effect.SNEAK) or
        player:hasStatusEffect(invaderXim.effect.DEODORIZE) or
        player:hasStatusEffect(invaderXim.effect.HIDE) or
        player:hasStatusEffect(invaderXim.effect.CAMOUFLAGE)
    then
        return invaderXim.msg.basic.ITEM_NO_USE_SNEAK, player:getEquipID(invaderXim.slot.RANGED)
    end

    local id = player:getEquipID(invaderXim.slot.AMMO)
    if
        id ~= invaderXim.item.BLANK_SOUL_PLATE and
        id ~= invaderXim.item.BLANK_HIGH_SPEED_SOUL_PLATE
    then
        return invaderXim.msg.basic.ITEM_NO_ITEMS_EQUIPPED
    end

    if player:getFreeSlotsCount() == 0 then
        return invaderXim.msg.basic.FULL_INVENTORY
    end
...
invaderXim.znm.soultrapper.onItemUse = function(target, item, player)
    -- Soul plate not guaranteed
    -- to validate long term: some posts hint at level correction on success rate vs. higher level mobs.
    if math.random(100) > invaderXim.znm.SOULTRAPPER_SUCCESS * invaderXim.znm.SOULPLATE_HS_MULT then
        -- todo, message should show to all in area
        player:timer(4000, function(playerArg)
            playerArg:messageBasic(invaderXim.msg.basic.SOULTRAPPER_FAILED)
        end)

        player:removeAmmo()
    else
        -- Determine Zeni starting value
        local zeni = invaderXim.znm.soultrapper.getZeniValue(target, player)

        -- Pick a skill totally at random...
        local skillIndex, skillEntry = utils.randomEntryIdx(invaderXim.pankration.feralSkills)
        local interestData = invaderXim.znm.soultrapper.packInterestData(target)
        ...
        -- todo, message should show to all in area
        player:timer(4000, function(playerArg)
            playerArg:messageBasic(invaderXim.msg.basic.SOULTRAPPER_SUCCESS, 0, invaderXim.item.SOUL_PLATE)
        end)
    end
end
...
invaderXim.znm.soultrapper.getZeniValue = function(target, player)
    ...
    if isFacing then
        zeni = zeni * invaderXim.znm.SOULPLATE_FACING_MULT
    end
    ...
    then -- Dahak and Aw'euvhi
        zeni = zeni + invaderXim.znm.SOULPLATE_UNIQUE_AMOUNT
    elseif
    ...
    elseif isNM then
        if level >= 80 then
            zeni = zeni * invaderXim.znm.SOULPLATE_HNM_MULT
        else
            zeni = zeni * invaderXim.znm.SOULPLATE_NM_MULT
        end
    end
...
invaderXim.znm.soultrapper.packInterestData = function(target)
    local data = 0
...
invaderXim.znm.ryo = invaderXim.znm.ryo or {}

-----------------------------------
-- onTrade
-----------------------------------

invaderXim.znm.ryo.onTrade = function(player, npc, trade)
    if npcUtil.tradeHasExactly(trade, invaderXim.item.SOUL_PLATE) then
        -- Cache the soulplate value on the player
        local item = trade:getItem(0)
        local zeni = invaderXim.znm.calculatePlateZeni(player, item:getSoulPlateData())
        invaderXim.znm.ryo.setTradedPlateValue(player, zeni)
...
invaderXim.znm.ryo.onTrigger = function(player, npc)
    if invaderXim.znm.playerHasSpokenToSanrakuBefore(player) then
...
invaderXim.znm.ryo.onEventUpdate = function(player, csid, option, npc)
    if csid == 914 then  -- Get approximate value of traded soulplate
        local zeniValue = invaderXim.znm.ryo.tradedPlateValue(player)

        invaderXim.znm.ryo.setTradedPlateValue(player, 0)
        player:updateEvent(zeniValue)
    elseif csid == 913 then
        if option == 200 then -- 'Sanraku's subject of interest'
            local param = invaderXim.znm.getSanrakusInterest()

            player:updateEvent(param, 0)
        elseif option == 201 then -- 'Sanraku's recommended fauna'
            local param = invaderXim.znm.getSanrakusFauna()
...
invaderXim.znm.ryo.onEventFinish = function(player, csid, option, npc)
end
...
invaderXim.znm.ryo.tradedPlateValue = function(player)
    return player:getLocalVar('[ZNM][Ryo]SoulPlateValue')
end

invaderXim.znm.ryo.setTradedPlateValue = function(player, zeni)
...
invaderXim.znm.sanraku = invaderXim.znm.sanraku or {}

-----------------------------------
-- onTrade
-----------------------------------

invaderXim.znm.sanraku.onTrade = function(player, npc, trade)
    if trade:getItemCount() == 1 then -- One soul plate or trophy at a time
        local item = trade:getItem(0)

        if trade:getItemId() == invaderXim.item.SOUL_PLATE then
            invaderXim.znm.sanraku.handleTradeWithPlate(player, npc, item)
        else -- Check Trophy trading (for ZNM seals)
            invaderXim.znm.sanraku.handleTradeWithTrophy(player, npc, item)
        end
    end
end
...
invaderXim.znm.sanraku.handleTradeWithPlate = function(player, npc, item)
    if not player:hasKeyItem(invaderXim.ki.RHAPSODY_IN_AZURE) then
        local tradeLimit = invaderXim.znm.SOULPLATE_TRADE_LIMIT

        if invaderXim.znm.sanraku.platesTradedToday(player) >= tradeLimit then
            player:showText(npc, ID.text.APPRECIATE_MORE, 1, invaderXim.item.SOUL_PLATE, tradeLimit)
            return
        end
    else -- If you have the KI, clear out the tracking vars!
        invaderXim.znm.resetDailyTrackingVars(player)
    end

    -- Cache the soulplate value on the player
    local zeni = invaderXim.znm.calculatePlateZeni(player, item:getSoulPlateData())
    invaderXim.znm.sanraku.setTradedPlateValue(player, zeni)
    invaderXim.znm.serverPlateTrades()

    player:startEvent(910, zeni)
end

invaderXim.znm.sanraku.platesTradedToday = function(player)
    local currentDay = VanadielYear() * 360 + VanadielDayOfTheYear()
    local storedDay  = invaderXim.znm.playerTradingDay(player)
    ...
    return invaderXim.znm.numberOfTradedPlates(player)
end

invaderXim.znm.sanraku.handleTradeWithTrophy = function(player, npc, item)
    local znmSeal = invaderXim.znm.TROPHIES[item:getID()]

    if znmSeal ~= nil then
        if player:hasKeyItem(znmSeal) then
            player:showText(npc, ID.text.SINGLE_TALLY)
        else
            invaderXim.znm.sanraku.setTradedTrophySeal(player, znmSeal)
...
invaderXim.znm.sanraku.onTrigger = function(player, npc)
    -- ZNM and Zeni Ineractions
    if invaderXim.znm.playerHasSpokenToSanrakuBefore(player) then
        if player:getCurrency('zeni_point') ~= 0 then
            local param = invaderXim.znm.sanraku.menu(player)
...
invaderXim.znm.sanraku.menu = function(player)
    -- Default: Tier 1 ZNMs + 'Don't Ask'
    -- (if bit = 0: add ZNM to Sanraku's Menu)
    local param = invaderXim.znm.DefaultMenu

    for bitmask, seal in pairs(invaderXim.znm.MENU_BITMASKS) do
...
invaderXim.znm.sanraku.onEventUpdate = function(player, csid, option, npc)
    if csid == 909 then
        ...
        elseif option >= 300 and option <= 302 then -- 'Gaining access to islets'
            invaderXim.znm.sanraku.handleGainingAccessToIslets(player, option)
        elseif option >= 100 and option <= 130 then -- Are you sure you want info on <ZNM_mob>?
            invaderXim.znm.sanraku.handleConfirmingDesiredZNMInfo(player, option)
        elseif option >= 400 and option <= 440 then -- Yes, I want info on <ZNM_mob>
            invaderXim.znm.sanraku.handleConfirmedZNMInfo(player, option)
        end
    end
end
...
invaderXim.znm.sanraku.handleGainingAccessToIslets = function(player, option)
    local zeniCost = 500 -- Base cost charged by Sanraku

    if player:hasKeyItem(invaderXim.keyItem.RHAPSODY_IN_AZURE) then -- Reduced zeni cost
        zeniCost = 50
    end

    -- Give the correct island's information + salt
    local keyItem = invaderXim.ki.SICKLEMOON_SALT + option - 300
...
invaderXim.znm.sanraku.handleConfirmingDesiredZNMInfo = function(player, option)
    -- Give the correct ZNM's zeni cost
    local diff      = option - 99
    local zeniCost = invaderXim.znm.getPopPrice(invaderXim.znm.POP_ITEMS[diff].mob, invaderXim.znm.POP_ITEMS[diff].tier)

    player:updateEvent(0, 0, 0, 0, 0, 0, zeniCost)
end

invaderXim.znm.sanraku.handleConfirmedZNMInfo = function(player, option)
    -- (440 because Warden's option is offset by 10 for some reason)
    local diff     = math.min(option - 399, 31) -- Determine the desired ZNM
    local popItem  = invaderXim.znm.POP_ITEMS[diff].item
    local znmTier  = invaderXim.znm.POP_ITEMS[diff].tier
    local mob      = invaderXim.znm.POP_ITEMS[diff].mob
    local zeniCost = invaderXim.znm.getPopPrice(mob, znmTier)
...
invaderXim.znm.sanraku.onEventFinish = function(player, csid, option, npc)
    if csid == 910 then
        invaderXim.znm.sanraku.handleCompletedTradeWithPlate(player)
    elseif csid == 908 then
        invaderXim.znm.setPlayerHasSpokenToSanrakuBefore(player)
    elseif csid == 912 then
        invaderXim.znm.sanraku.handleCompletedTradeWithTrophy(player)
    end
end
...
invaderXim.znm.sanraku.handleCompletedTradeWithPlate = function(player)
    player:tradeComplete()
    invaderXim.znm.setPlayerTradingDay(player, VanadielYear() * 360 + VanadielDayOfTheYear())
    invaderXim.znm.incrementTradedPlates(player)

    local zeniValue = invaderXim.znm.sanraku.tradedPlateValue(player)
    invaderXim.znm.sanraku.setTradedPlateValue(player, 0)

    player:addCurrency('zeni_point', zeniValue)
end

invaderXim.znm.sanraku.handleCompletedTradeWithTrophy = function(player)
    player:tradeComplete()
    player:addKeyItem(invaderXim.znm.sanraku.tradedTrophySeal(player))
    invaderXim.znm.sanraku.setTradedTrophySeal(player, 0)
end
...
invaderXim.znm.sanraku.tradedPlateValue = function(player)
    return player:getLocalVar('[ZNM][Sanraku]SoulPlateValue')
end

invaderXim.znm.sanraku.setTradedPlateValue = function(player, zeni)
    player:setLocalVar('[ZNM][Sanraku]SoulPlateValue', zeni)
end

invaderXim.znm.sanraku.tradedTrophySeal = function(player)
    return player:getLocalVar('[ZNM]TrophyTrade')
end

invaderXim.znm.sanraku.setTradedTrophySeal = function(player, trophy)
...
invaderXim.znm.getPopPrice = function(mob, znmTier)
    ...
    if
        ...
        popCost == 0
    then
        popCost = invaderXim.znm.ZNM_popCostS[znmTier].minPrice
...
invaderXim.znm.updatePopPrice = function(mob, znmTier)
    if not invaderXim.znm.ZNM_STATIC_POP_PRICES then
        local popCost = math.min(invaderXim.znm.getPopPrice(mob, znmTier) + invaderXim.znm.ZNM_popCostS[znmTier].addedPrice,
                invaderXim.znm.ZNM_popCostS[znmTier].maxPrice)
        SetServerVariable('[ZNM][' .. mob .. ']PopCost', popCost)
    end
end

-- Prices decay over time (called every 2 hours)
invaderXim.znm.ZNMPopPriceDecay = function()
    if not invaderXim.znm.ZNM_STATIC_POP_PRICES then
        local popCost = 0
        local mob     = nil
        local znmTier = 1
        ...
        for i = 1, 31 do
            mob     = invaderXim.znm.POP_ITEMS[i].mob
            znmTier = invaderXim.znm.POP_ITEMS[i].tier
            popCost = math.max(invaderXim.znm.getPopPrice(mob, znmTier) - invaderXim.znm.ZNM_popCostS[znmTier].decayPrice,
                    invaderXim.znm.ZNM_popCostS[znmTier].minPrice)
...
invaderXim.znm.playerHasSpokenToSanrakuBefore = function(player)
    return player:getVar('ZeniStatus') >= 1
end

invaderXim.znm.setPlayerHasSpokenToSanrakuBefore = function(player)
    player:setVar('ZeniStatus', 1)
end

invaderXim.znm.playerTradingDay = function(player)
    return player:getVar('[ZNM][Sanraku]TradingDay')
end

invaderXim.znm.setPlayerTradingDay = function(player, day)
    player:setVar('[ZNM][Sanraku]TradingDay', day)
end

invaderXim.znm.numberOfTradedPlates = function(player)
    return player:getVar('[ZNM][Sanraku]TradedPlates')
end

invaderXim.znm.incrementTradedPlates = function(player)
    player:addVar('[ZNM][Sanraku]TradedPlates', 1)
end

invaderXim.znm.resetDailyTrackingVars = function(player)
