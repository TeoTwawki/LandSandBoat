...
itemObject.onItemEquip = function(target, item)
    target:addListener('DEFEATED_MOB', 'CHAOSBRINGER_KILLS', function(mob, player, optParams)
        if
            (player:getQuestStatus(invaderXim.questLog.BASTOK, invaderXim.quest.id.bastok.BLADE_OF_DARKNESS) == invaderXim.questStatus.QUEST_ACCEPTED or
            player:getQuestStatus(invaderXim.questLog.BASTOK, invaderXim.quest.id.bastok.BLADE_OF_DEATH) == invaderXim.questStatus.QUEST_ACCEPTED) and
