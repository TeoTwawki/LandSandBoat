...
local mission = Mission:new(invaderXim.mission.log_id.BASTOK, invaderXim.mission.id.bastok.GEOLOGICAL_SURVEY)
        ...
        [invaderXim.zone.DANGRUF_WADI] =
        {
            onEventFinish =
            {
                [10] = function(player, csid, option, npc)
                    if player:hasKeyItem(invaderXim.ki.BLUE_ACIDITY_TESTER) then
                        player:delKeyItem(invaderXim.ki.BLUE_ACIDITY_TESTER)
                        npcUtil.giveKeyItem(player, invaderXim.ki.RED_ACIDITY_TESTER)
                    end
                end,
        ...
        [invaderXim.zone.METALWORKS] =
        {
            ['Cid'] =
            {
                onTrigger = function(player, npc)
                    if player:hasKeyItem(invaderXim.ki.RED_ACIDITY_TESTER) then
                        return mission:progressEvent(504)
                    elseif not player:hasKeyItem(invaderXim.ki.BLUE_ACIDITY_TESTER) then
            ...
            onEventFinish =
            {
                [503] = function(player, csid, option, npc)
                    npcUtil.giveKeyItem(player, invaderXim.ki.BLUE_ACIDITY_TESTER)
                end,

                [504] = function(player, csid, option, npc)
                    if mission:complete(player) then
                        player:delKeyItem(invaderXim.ki.RED_ACIDITY_TESTER)
                    end
                end,
            },
        },
