...
local arrapagoID  = zones[invaderXim.zone.ARRAPAGO_REEF]
local bhaflauID   = zones[invaderXim.zone.BHAFLAU_THICKETS]
local caedarvaID  = zones[invaderXim.zone.CAEDARVA_MIRE]
local zhayolmID   = zones[invaderXim.zone.MOUNT_ZHAYOLM]
-----------------------------------

local mission = Mission:new(invaderXim.mission.log_id.TOAU, invaderXim.mission.id.toau.IMMORTAL_SENTRIES)

local function handlePackage(player)
    player:delKeyItem(invaderXim.ki.SUPPLIES_PACKAGE)
...
mission.reward =
{
    keyItem     = invaderXim.ki.PSC_WILDCAT_BADGE,
    title       = invaderXim.title.PRIVATE_SECOND_CLASS,
    nextMission = { invaderXim.mission.log_id.TOAU, invaderXim.mission.id.toau.PRESIDENT_SALAHEEM },
}
...
{
    {
        ...
        [invaderXim.zone.AHT_URHGAN_WHITEGATE] =
        {
            ['Naja_Salaheem'] =
            {
                onTrigger = function(player, npc)
                    if not player:hasKeyItem(invaderXim.ki.SUPPLIES_PACKAGE) then
        ...
        [invaderXim.zone.ARRAPAGO_REEF] =
        {
            ...
            {
                onTrigger = function(player, npc)
                    if player:hasKeyItem(invaderXim.ki.SUPPLIES_PACKAGE) then
            ...
            onEventFinish =
            {
                [5] = function(player, csid, option, npc)
                    if option == 1 then
                        ...
                        player:messageSpecial(arrapagoID.text.HAND_OVER_TO_IMMORTAL, invaderXim.ki.SUPPLIES_PACKAGE)
        ...
        [invaderXim.zone.BHAFLAU_THICKETS] =
        {
            ...
            onEventFinish =
            {
                [5] = function(player, csid, option, npc)
                    if option == 1 then
                        handlePackage(player)
                        player:messageSpecial(bhaflauID.text.HAND_OVER_TO_IMMORTAL, invaderXim.ki.SUPPLIES_PACKAGE)
        ...
        [invaderXim.zone.CAEDARVA_MIRE] =
        {
            ...
            onEventFinish =
            {
                [5] = function(player, csid, option, npc)
                    if option == 1 then
                        handlePackage(player)
                        player:messageSpecial(caedarvaID.text.HAND_OVER_TO_IMMORTAL, invaderXim.ki.SUPPLIES_PACKAGE)
        ...
        [invaderXim.zone.MOUNT_ZHAYOLM] =
        {
            ...
            onEventFinish =
            {
                [4] = function(player, csid, option, npc)
                    if option == 1 then
                        handlePackage(player)
                        player:messageSpecial(zhayolmID.text.HAND_OVER_TO_IMMORTAL, invaderXim.ki.SUPPLIES_PACKAGE)
