...
local quest = Quest:new(invaderXim.questLog.BASTOK, invaderXim.quest.id.bastok.THE_WALLS_OF_YOUR_MIND)
    ...
    -- Section: Quest available
    {
        check = function(player, status, vars)
            return status == invaderXim.questStatus.QUEST_AVAILABLE and
                player:canEquipItem(invaderXim.item.KNUCKLES_OF_TRIALS, true) and
                player:getCharSkillLevel(invaderXim.skill.HAND_TO_HAND) / 10 >= 250 and
        ...
        [invaderXim.zone.PORT_BASTOK] =
        {
            ...
            onEventFinish =
            {
                [286] = function(player, csid, option, npc)
                    if
                        player:hasItem(invaderXim.item.KNUCKLES_OF_TRIALS) or
                        npcUtil.giveItem(player, invaderXim.item.KNUCKLES_OF_TRIALS)
                    then
        ...
        [invaderXim.zone.PORT_BASTOK] =
        {
            ['Oggbi'] =
            {
                onTrigger = function(player, npc)
                    if player:hasKeyItem(invaderXim.ki.ANNALS_OF_TRUTH) then
                        ...
                        local hideReacquireMenuItem = (player:hasItem(invaderXim.item.KNUCKLES_OF_TRIALS) or player:hasKeyItem(invaderXim.ki.MAP_TO_THE_ANNALS_OF_TRUTH)) and 1 or 0
                        return quest:event(287, hideReacquireMenuItem)
                    end
                end,

                onTrade = function(player, npc, trade)
                    if npcUtil.tradeHasExactly(trade, invaderXim.item.KNUCKLES_OF_TRIALS) then
            ...
            onEventFinish =
            {
                [287] = function(player, csid, option, npc)
                    if option == 1 and not player:hasItem(invaderXim.item.KNUCKLES_OF_TRIALS) then
                        npcUtil.giveItem(player, invaderXim.item.KNUCKLES_OF_TRIALS)
                    elseif option == 2 then
                        player:delQuest(invaderXim.questLog.BASTOK, invaderXim.quest.id.bastok.THE_WALLS_OF_YOUR_MIND)
                ...
                [290] = function(player, csid, option, npc)
                    if quest:complete(player) then
                        ...
                        player:delKeyItem(invaderXim.ki.WEAPON_TRAINING_GUIDE)
                        player:addLearnedWeaponskill(invaderXim.wsUnlock.ASURAN_FISTS)
