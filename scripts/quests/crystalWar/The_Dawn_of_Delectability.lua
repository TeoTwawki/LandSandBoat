...
local quest = Quest:new(invaderXim.questLog.CRYSTAL_WAR, invaderXim.quest.id.crystalWar.THE_DAWN_OF_DELECTABILITY)

quest.reward =
{
    item = invaderXim.item.TRAINEE_KNIFE,
}
...
quest.sections =
{
        ...
        [invaderXim.zone.WINDURST_WATERS] =
        {
            ['Ranpi-Monpi'] =
            {
                onTrade = function(player, npc, trade)
                    if
                        npcUtil.tradeHasExactly(trade, { invaderXim.item.PEPPERONI, invaderXim.item.WALNUT, invaderXim.item.DRAGON_FRUIT, invaderXim.item.BASTORE_SWEEPER })
                    then
    ...
    {
        check = function(player, status, vars)
            return status == invaderXim.questStatus.QUEST_ACCEPTED and vars.Prog == 3 and
        ...
        [invaderXim.zone.WINDURST_WATERS] =
        {
            ...
            onEventFinish =
            {
                [980] = function(player, csid, option, npc)
                    npcUtil.giveKeyItem(player, invaderXim.ki.RANPI_MONPI_SPECIALTY)
        ...
        [invaderXim.zone.WINDURST_WATERS_S] =
        {
            ...
            onEventFinish =
            {
                [118] = function(player, csid, option, npc)
                    player:delKeyItem(invaderXim.ki.RANPI_MONPI_SPECIALTY)
                    npcUtil.giveKeyItem(player, invaderXim.ki.CULINARY_KNIFE)
            ...
            onEventFinish =
            {
                [981] = function(player, csid, option, npc)
                    if quest:complete(player) then
                        player:delKeyItem(invaderXim.ki.CULINARY_KNIFE)
                    end
                end,
            },
        },
    ...
    {
        check = function(player, status, vars)
            return status == invaderXim.questStatus.QUEST_COMPLETED or vars.Prog == 5
        end,
