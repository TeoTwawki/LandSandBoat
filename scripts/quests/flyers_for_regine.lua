...
quests.flyers_for_regine.onTriggerAreaEnter = function(player, triggerArea)
    if player:getQuestStatus(invaderXim.questLog.SANDORIA, invaderXim.quest.id.sandoria.FLYERS_FOR_REGINE) == invaderXim.questStatus.QUEST_ACCEPTED then
...
quests.flyers_for_regine.onTrade = function(player, npc, trade, ffrId)
    if
        player:getQuestStatus(invaderXim.questLog.SANDORIA, invaderXim.quest.id.sandoria.FLYERS_FOR_REGINE) == invaderXim.questStatus.QUEST_ACCEPTED and
        npcUtil.tradeHas(trade, invaderXim.item.MAGICMART_FLYER)
    then
        local zoneId = player:getZoneID()
        local ID = zones[zoneId]
        local mask = player:getCharVar('[ffr]deliveryMask')
