...
local quest = Quest:new(invaderXim.questLog.OTHER_AREAS, invaderXim.quest.id.otherAreas.UNENDING_CHASE)

quest.reward =
{
    fame     = 120,
    fameArea = invaderXim.fameArea.WINDURST,
    title    = invaderXim.title.TWO_STAR_PURVEYOR,
    gil      = 2100,
}
    ...
    -- Section: Quest is available.
    {
        check = function(player, status, vars)
            return status == invaderXim.questStatus.QUEST_AVAILABLE and
                player:getQuestStatus(invaderXim.questLog.OTHER_AREAS, invaderXim.quest.id.otherAreas.WAY_OF_THE_COOK) == invaderXim.questStatus.QUEST_COMPLETED
        end,
                ...
                onTrigger = function(player, npc)
                    ...
                    then
                        if quest:getVar(player, 'Prog') == 0 then
                            return quest:progressEvent(82, invaderXim.item.PUFFBALL) -- Unending Chase starting event.
                        else
                            return quest:progressEvent(84, invaderXim.item.PUFFBALL) -- Unending Chase starting event after rejecting it.
                        end
                    else
        ...
        [invaderXim.zone.MHAURA] =
        {
            ['Rycharde'] =
            {
                ...
                onTrade = function(player, npc, trade)
                    if npcUtil.tradeHasExactly(trade, { invaderXim.item.PUFFBALL }) then
