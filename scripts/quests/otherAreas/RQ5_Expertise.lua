...
local quest = Quest:new(invaderXim.questLog.OTHER_AREAS, invaderXim.quest.id.otherAreas.EXPERTISE)

quest.reward =
{
    item  = invaderXim.item.TABLEWARE_SET,
    title = invaderXim.title.THREE_STAR_PURVEYOR,
}
    ...
    -- Section: Quest is available.
    {
        check = function(player, status, vars)
            return status == invaderXim.questStatus.QUEST_AVAILABLE and
                player:getQuestStatus(invaderXim.questLog.OTHER_AREAS, invaderXim.quest.id.otherAreas.HIS_NAME_IS_VALGEIR) == invaderXim.questStatus.QUEST_COMPLETED
        end,
            ...
            ['Take'] =
            {
                onTrigger = function(player, npc)
                    if quest:getVar(player, 'Prog') == 3 then
                        return quest:progressEvent(62, invaderXim.item.TABLEWARE_SET) -- Give dish from Valgeir.
                    else
            ...
            onEventFinish =
            {
                [62] = function(player, csid, option, npc)
                    if quest:complete(player) then
                        player:delKeyItem(invaderXim.ki.LAND_CRAB_BISQUE) -- Give Land Crab Bisque from Valgeir.
                        player:messageSpecial(mhauraID.text.KEYITEM_OBTAINED + 1, invaderXim.ki.LAND_CRAB_BISQUE)
        ...
        [invaderXim.zone.SELBINA] =
        {
            ['Valgeir'] =
            {
                onTrigger = function(player, npc)
                    ...
                    if questProgress == 0 then
                        return quest:progressEvent(102, invaderXim.item.SCREAM_FUNGUS, invaderXim.item.SLICE_OF_LAND_CRAB_MEAT) -- Ask for ingredients to cook.
                ...
                onTrade = function(player, npc, trade)
                    if npcUtil.tradeHasExactly(trade, { invaderXim.item.SCREAM_FUNGUS, invaderXim.item.SLICE_OF_LAND_CRAB_MEAT }) then
            ...
            onEventFinish =
            {
                ...
                [105] = function(player, csid, option, npc)
                    ...
                    npcUtil.giveKeyItem(player, invaderXim.ki.LAND_CRAB_BISQUE)
                end,
            },
        },
    },
