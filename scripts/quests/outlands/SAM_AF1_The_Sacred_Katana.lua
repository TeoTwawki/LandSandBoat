...
local quest = Quest:new(invaderXim.questLog.OUTLANDS, invaderXim.quest.id.outlands.THE_SACRED_KATANA)

quest.reward =
{
    fame = 20,
    fameArea = invaderXim.fameArea.NORG,
    item = invaderXim.item.MAGOROKU,
}
...
{
    {
        check = function(player, status, vars)
            return status == invaderXim.questStatus.QUEST_AVAILABLE and
                player:hasCompletedQuest(invaderXim.questLog.OUTLANDS, invaderXim.quest.id.outlands.FORGE_YOUR_DESTINY) and
...

    {
        ...
        [invaderXim.zone.NORG] =
        {
            ['Jaucribaix'] =
            {
                onTrade = function(player, npc, trade)
                    if
                        player:hasKeyItem(invaderXim.ki.HANDFUL_OF_CRYSTAL_SCALES) and
                        npcUtil.tradeHasExactly(trade, invaderXim.item.MUMEITO)
                    then
            ...
            ['Ranemaud'] =
            {
                onTrade = function(player, npc, trade)
                    if
                        not player:findItem(invaderXim.item.MUMEITO) and
                ...
                onTrigger = function(player, npc)
                    if not player:findItem(invaderXim.item.MUMEITO) then
            ...
            onEventFinish =
            {
                ...
                [141] = function(player, csid, option, npc)
                    if quest:complete(player) then
                        player:confirmTrade()
                        player:delKeyItem(invaderXim.ki.HANDFUL_OF_CRYSTAL_SCALES)
                ...
                [145] = function(player, csid, option, npc)
                    if npcUtil.giveItem(player, invaderXim.item.MUMEITO) then
                        player:confirmTrade()
                    end
                end,
            },
        ...
        [invaderXim.zone.THE_SANCTUARY_OF_ZITAH] =
        {
            ...
            ['qm3'] =
            {
                onTrade = function(player, npc, trade)
                    if
                        npcUtil.tradeHasExactly(trade, invaderXim.item.SACK_OF_FISH_BAIT) and
                ...
                onTrigger = function(player, npc)
                    if
                        ...
                        not player:hasKeyItem(invaderXim.ki.HANDFUL_OF_CRYSTAL_SCALES)
                    then
                        return quest:keyItem(invaderXim.ki.HANDFUL_OF_CRYSTAL_SCALES)
                    end
                end,
            },
        },
