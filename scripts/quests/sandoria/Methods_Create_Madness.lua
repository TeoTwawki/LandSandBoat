...
local seaSerpentGrottoID = zones[invaderXim.zone.SEA_SERPENT_GROTTO]
-----------------------------------

local quest = Quest:new(invaderXim.questLog.SANDORIA, invaderXim.quest.id.sandoria.METHODS_CREATE_MADNESS)
    ...
    -- Talk to Balasiel in Southern San d'Oria who will give you the Spear of Trials and the Weapon Training Guide.
    {
        check = function(player, status, vars)
            return status == invaderXim.questStatus.QUEST_AVAILABLE and
                player:canEquipItem(invaderXim.item.SPEAR_OF_TRIALS, true) and
                player:getCharSkillLevel(invaderXim.skill.POLEARM) / 10 >= 240 and
        ...
        [invaderXim.zone.SOUTHERN_SAN_DORIA] =
        {
            ...
            onEventFinish =
            {
                [8] = function(player, csid, option, npc)
                    if
                        option == 1 and
                        (player:hasItem(invaderXim.item.SPEAR_OF_TRIALS) or npcUtil.giveItem(player, invaderXim.item.SPEAR_OF_TRIALS))
                    then
        ...
        [invaderXim.zone.SOUTHERN_SAN_DORIA] =
        {
            ['Balasiel'] =
            {
                onTrigger = function(player, npc)
                    ...
                    elseif player:hasKeyItem(invaderXim.ki.MAP_TO_THE_ANNALS_OF_TRUTH) then
                        ...
                        return quest:event(11, player:hasItem(invaderXim.item.SPEAR_OF_TRIALS) and 1 or 0) -- cont 1
                    end
                end,

                onTrade = function(player, npc, trade)
                    if npcUtil.tradeHasExactly(trade, invaderXim.item.SPEAR_OF_TRIALS) then
            ...
            onEventFinish =
            {
                [11] = function(player, csid, option, npc)
                    if option == 1 and not player:hasItem(invaderXim.item.SPEAR_OF_TRIALS) then
                        npcUtil.giveItem(player, invaderXim.item.SPEAR_OF_TRIALS)
                    elseif option == 2 then
                        player:delQuest(invaderXim.questLog.SANDORIA, invaderXim.quest.id.sandoria.METHODS_CREATE_MADNESS)
                ...
                [13] = function(player, csid, option, npc)
                    if quest:complete(player) then
                        ...
                        player:delKeyItem(invaderXim.ki.WEAPON_TRAINING_GUIDE)
                        player:addLearnedWeaponskill(invaderXim.wsUnlock.IMPULSE_DRIVE)
