...
local quest = Quest:new(invaderXim.questLog.WINDURST, invaderXim.quest.id.windurst.BLUE_RIBBON_BLUES)

quest.reward =
{
    fame  = 140,
    fameArea = invaderXim.fameArea.WINDURST,
    title = invaderXim.title.GHOSTIE_BUSTER,
}
...
{
    {
        check = function(player, status, vars)
            return status == invaderXim.questStatus.QUEST_AVAILABLE and
                player:hasCompletedQuest(invaderXim.questLog.WINDURST, invaderXim.quest.id.windurst.WATER_WAY_TO_GO) and
        ...
        [invaderXim.zone.WINDURST_WATERS] =
        {
            ['Kerutoto'] =
            {
                onTrade = function(player, npc, trade)
                    if npcUtil.tradeHasExactly(trade, invaderXim.item.PURPLE_RIBBON) then
        ...
        [invaderXim.zone.WINDURST_WOODS] =
        {
            ...
            {
                onTrigger = function(player, npc)
                    if not player:findItem(invaderXim.item.PURPLE_RIBBON) then
                        local questProgress = quest:getVar(player, 'Prog')

                        if questProgress == 1 then
                            return quest:progressEvent(376, 0, invaderXim.item.PURPLE_RIBBON)
                        else
                            return quest:progressEvent(377, 0, invaderXim.item.PURPLE_RIBBON)
                        end
                    else
            ...
            onEventFinish =
            {
                [376] = function(player, csid, option, npc)
                    if option == 1 then
                        npcUtil.giveItem(player, invaderXim.item.PURPLE_RIBBON)
                    end
                end,
    ...
    -- to accepting, the Prog variable is reset to 0 when transitioning to this block.
    {
        ...
        [invaderXim.zone.THE_ELDIEME_NECROPOLIS] =
        {
            ['Hume_Bones'] =
            {
                onTrade = function(player, npc, trade)
                    if
                        npcUtil.tradeHasExactly(trade, invaderXim.item.PURPLE_RIBBON) and
                ...
                onTrigger = function(player, npc)
                    if quest:getLocalVar(player, 'Prog') == 2 then
                        ...
                        if npcUtil.giveItem(player, invaderXim.item.BLUE_RIBBON, { silent = true }) then
                            quest:setVar(player, 'Prog', 3)
                            return quest:messageSpecial(eldiemeID.text.ITEM_OBTAINED, invaderXim.item.BLUE_RIBBON)
                        else
                            return quest:messageSpecial(eldiemeID.text.ITEM_CANNOT_BE_OBTAINED, invaderXim.item.BLUE_RIBBON)
                        end
                    end
                end,
            },
        ...
        [invaderXim.zone.WINDURST_WATERS] =
        {
            ['Kerutoto'] =
            {
                ...
                onTrigger = function(player, npc)
                    ...
                    elseif questProgress == 1 then
                        if not player:findItem(invaderXim.item.PURPLE_RIBBON) then
                            return quest:progressEvent(366, 0, invaderXim.item.PURPLE_RIBBON)
                        else
                            return quest:progressEvent(361, 0, invaderXim.item.PURPLE_RIBBON)
                        end
            ...
            onEventFinish =
            {
                [360] = function(player, csid, option, npc)
                    if npcUtil.giveItem(player, invaderXim.item.PURPLE_RIBBON) then
                        quest:setVar(player, 'Prog', 1)
                    end
                end,
            ...
            ['Roberta'] =
            {
                onTrigger = function(player, npc)
                    if
                        ...
                        not player:findItem(invaderXim.item.PURPLE_RIBBON)
                    then
                        return quest:progressEvent(377, 0, invaderXim.item.PURPLE_RIBBON)
                    else
