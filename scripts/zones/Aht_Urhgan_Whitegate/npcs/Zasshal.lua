...
entity.onTrigger = function(player, npc)
    ...
    if player:hasKeyItem(invaderXim.ki.REMNANTS_PERMIT) then
        player:startEvent(821)
--[[    elseif player:getCurrentMission(invaderXim.mission.log_id.TOAU) > invaderXim.mission.id.toau.GUESTS_OF_THE_EMPIRE and player:getMainLvl() >= 65 then
...
entity.onEventUpdate = function(player, csid, option, npc)
    if
        ...
        option == 10 and
        player:getAssaultPoint(invaderXim.assault.assaultArea.LEUJAOAM_SANCTUM) >= 500
    then
        player:setLocalVar('SalvageValid', 1)
    elseif
        (csid == 818 or csid == 820) and
        option == 11 and
        player:getAssaultPoint(invaderXim.assault.assaultArea.MAMOOL_JA_TRAINING_GROUNDS) >= 500
    then
        player:setLocalVar('SalvageValid', 2)
    elseif
        (csid == 818 or csid == 820) and
        option == 12 and
        player:getAssaultPoint(invaderXim.assault.assaultArea.LEBROS_CAVERN) >= 500
    then
        player:setLocalVar('SalvageValid', 3)
    elseif
        (csid == 818 or csid == 820) and
        option == 13 and
        player:getAssaultPoint(invaderXim.assault.assaultArea.PERIQIA) >= 500
    then
        player:setLocalVar('SalvageValid', 4)
    elseif
        (csid == 818 or csid == 820) and
        option == 14 and
        player:getAssaultPoint(invaderXim.assault.assaultArea.ILRUSI_ATOLL) >= 500
    then
...
entity.onEventFinish = function(player, csid, option, npc)
    ...
    if (csid == 818 or csid == 820) and option == 100 then
        if player:getLocalVar('SalvageValid') == 1 then
            player:addKeyItem(invaderXim.ki.REMNANTS_PERMIT)
