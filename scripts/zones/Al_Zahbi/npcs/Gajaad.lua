...
entity.onTrade = function(player, npc, trade)
    ...
    local tradeCount = trade:getItemQty(invaderXim.item.IMPERIAL_BRONZE_PIECE)
    ...
    if tradeCount > 0 and tradeCount == trade:getItemCount() then
        if walahraCoinCount + tradeCount >= 1000 then -- give player turban, donated over 1000
            if player:addItem(invaderXim.item.WALAHRA_TURBAN) then
                player:setCharVar('walahraCoinCount', walahraCoinCount - (1000 - tradeCount))
                player:tradeComplete()
                player:messageSpecial(ID.text.ITEM_OBTAINED, invaderXim.item.WALAHRA_TURBAN)
                player:startEvent(102, invaderXim.item.IMPERIAL_BRONZE_PIECE, 0, tradeCount)
            else
                player:messageSpecial(ID.text.ITEM_CANNOT_BE_OBTAINED, invaderXim.item.WALAHRA_TURBAN)
            end
            ...
            if tradeCount >= 100 then -- give bonus walahra water - only one water per trade, regardless of the amount.
                ...
                player:addItem(invaderXim.item.FLASK_OF_WALAHRA_WATER)
                player:messageSpecial(ID.text.ITEM_OBTAINED, invaderXim.item.FLASK_OF_WALAHRA_WATER)
                player:startEvent(102, invaderXim.item.IMPERIAL_BRONZE_PIECE, 0, tradeCount)
            else
                player:tradeComplete()
                player:setCharVar('walahraCoinCount', walahraCoinCount + tradeCount)
...
entity.onTrigger = function(player, npc)
    ...
    player:startEvent(102, invaderXim.item.IMPERIAL_BRONZE_PIECE)
end
