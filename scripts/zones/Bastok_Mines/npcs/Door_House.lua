...
entity.onTrade = function(player, npc, trade)
    if npc:getID() == ID.npc.LELEROON_BLUE_DOOR then
        ...
        if
            letterBlue == 2 and
            npcUtil.tradeHas(trade, { invaderXim.item.MYTHRIL_SHEET, invaderXim.item.SQUARE_OF_KARAKUL_LEATHER, invaderXim.item.SQUARE_OF_LM_BUFFALO_LEATHER, invaderXim.item.SQUARE_OF_WOLF_FELT }) -- mythril sheet, karakul leather, laminated buffalo leather, wolf felt
        then
            player:startEvent(521) -- accepts materials, now bring me 4 imperial mythril pieces

        elseif
            letterBlue == 3 and
            npcUtil.tradeHas(trade, { { invaderXim.item.IMPERIAL_MYTHRIL_PIECE, 4 } }) -- 4 imperial mythril pieces
        then
...
entity.onTrigger = function(player, npc)
    if npc:getID() == ID.npc.LELEROON_BLUE_DOOR then
        ...
        if player:hasKeyItem(invaderXim.ki.LELEROONS_LETTER_BLUE) then
...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 519 then
        ...
        player:delKeyItem(invaderXim.ki.LELEROONS_LETTER_BLUE)
    ...
    elseif
        csid == 522 and
        npcUtil.giveItem(player, invaderXim.item.CORSAIRS_BOTTES)
    then
