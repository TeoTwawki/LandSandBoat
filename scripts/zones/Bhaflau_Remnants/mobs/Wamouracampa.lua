...
entity.onMobSpawn = function(mob)
    ...
    mob:addListener('ROAM_TICK', 'WAMOURACAMPA_ROAM', function(mobArg)
        ...
        if GetSystemTime() > roamTime then
            if mobArg:getAnimationSub() == 0 then
                ...
                mobArg:setMod(invaderXim.mod.DMGPHYS, 0)
                mobArg:setMod(invaderXim.mod.DMGRANGE, 0)
                mobArg:setMod(invaderXim.mod.UDMGMAGIC, 1500)
                mobArg:setMod(invaderXim.mod.UDMGBREATH, 150)
                mobArg:setLocalVar('damageThreshold', math.random(5, 15) / 100 * mobArg:getMaxHP())
            else
                mobArg:setAnimationSub(0)
                mobArg:setMod(invaderXim.mod.DMGPHYS, -25)
                mobArg:setMod(invaderXim.mod.DMGRANGE, -25)
                mobArg:setMod(invaderXim.mod.UDMGMAGIC, 2000)
                mobArg:setMod(invaderXim.mod.UDMGBREATH, 200)
    ...
    mob:addListener('AFTER_SPAWN', 'WAMOURACAMPA_AFTER_SPAWN', function(mobArg)
        mobArg:setDelay(330)
        mobArg:setMod(invaderXim.mod.ATT, 101)
        mobArg:setMod(invaderXim.mod.MAIN_DMG_RATING, -45)
    end)
    ...
    mob:addListener('AFTER_SPAWN', 'WAMOURACAMPA_AFTER_SPAWN', function(mobArg)
        ...
        mobArg:setMod(invaderXim.mod.ATT, 101)
        mobArg:setMod(invaderXim.mod.MAIN_DMG_RATING, -45)
        mobArg:setMod(invaderXim.mod.INT, -35)
        mobArg:setMod(invaderXim.mod.MATT, -15)
    end)
end
