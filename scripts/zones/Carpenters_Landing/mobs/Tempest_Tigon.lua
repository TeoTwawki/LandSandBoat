...
entity.onAdditionalEffect = function(mob, target, damage)
    if math.random(1, 100) <= 50 then
        return invaderXim.mob.onAddEffect(mob, target, damage, invaderXim.mob.ae.ENAERO, { chance = 50 })
    else
        return invaderXim.mob.onAddEffect(mob, target, damage, invaderXim.mob.ae.ENWATER, { chance = 50 })
    end
end

entity.onMobDeath = function(mob, player, optParams)
    invaderXim.hunts.checkHunt(mob, player, 168)
end
