...
entity.onAdditionalEffect = function(mob, target, damage)
    return invaderXim.mob.onAddEffect(mob, target, damage, invaderXim.mob.ae.ENAERO, { chance = 100, power = math.random(75, 125) })
end
    ...
    if mob:getLocalVar('healTimer') < os.time() then
        ...
        for i = 1, 4 do
            ...
            if elemental and elemental:isAlive() then
                ...
                if spellTarget then
                    elemental:castSpell(invaderXim.magic.spell.AERO_IV, spellTarget)
                    mob:setLocalVar('healTimer', os.time() + math.random(30, 60))
                    break
                end
            end
