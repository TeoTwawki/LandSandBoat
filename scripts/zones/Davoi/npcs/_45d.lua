...
entity.onTrigger = function(player, npc)
    if npc:getAnimation() == invaderXim.animation.CLOSE_DOOR then
        if player:hasKeyItem(invaderXim.ki.CRIMSON_ORB) then
...
entity.onEventFinish = function(player, csid, option, npc)
    if
        csid == 42 and
        option == 0 and
        npc:getAnimation() == invaderXim.animation.CLOSE_DOOR
    then
        ...
        npc:timer(20000, function(npcArg)
            npcArg:setAnimation(invaderXim.animation.CLOSE_DOOR)
        end)
    end
end
