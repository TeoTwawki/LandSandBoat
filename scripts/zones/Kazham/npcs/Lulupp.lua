...
entity.onTrade = function(player, npc, trade)
    ...
    if opoOpoAndIStatus == invaderXim.questStatus.QUEST_ACCEPTED then
        if progress == 0 or failed == 1 then
            if trade:hasItemQty(invaderXim.item.BROKEN_MITHRAN_FISHING_ROD, 1) then -- first or second time trading correctly
...
entity.onTrigger = function(player, npc)
    ...
    if
        ...
        opoOpoAndIStatus == invaderXim.questStatus.QUEST_AVAILABLE
    then
        player:startEvent(217, 0, 483)  -- 483 broken mithran fishing rod
    elseif opoOpoAndIStatus == invaderXim.questStatus.QUEST_ACCEPTED then
        if retry == 1 then
...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 217 and option == 1  then                   -- Opo Opo and I quest start CS
        player:addQuest(invaderXim.questLog.OUTLANDS, invaderXim.quest.id.outlands.THE_OPO_OPO_AND_I)
