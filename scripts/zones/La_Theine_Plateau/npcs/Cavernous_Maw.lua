...
entity.onTrigger = function(player, npc)
    if invaderXim.settings.main.ENABLE_ABYSSEA == 1 and player:getMainLvl() >= 30 then
        ...
        if
            ...
            player:getQuestStatus(invaderXim.questLog.ABYSSEA, invaderXim.quest.id.abyssea.A_GOLDSTRUCK_GIGAS) == invaderXim.questStatus.QUEST_AVAILABLE
        then
            player:startEvent(9)
        else
...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 9 then
        player:addQuest(invaderXim.questLog.ABYSSEA, invaderXim.quest.id.abyssea.A_GOLDSTRUCK_GIGAS)
    elseif csid == 10 then
        -- Killed Briareus
