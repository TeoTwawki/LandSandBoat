...
entity.onTrigger = function(player, npc)
    ...
    if player:getQuestStatus(invaderXim.questLog.JEUNO, invaderXim.quest.id.jeuno.THE_WONDER_MAGIC_SET) == invaderXim.questStatus.QUEST_AVAILABLE then
        player:startEvent(34) -- Base Standard CS & dialog
    elseif player:getQuestStatus(invaderXim.questLog.JEUNO, invaderXim.quest.id.jeuno.COOKS_PRIDE) ~= invaderXim.questStatus.QUEST_COMPLETED then
    ...
    elseif player:getQuestStatus(invaderXim.questLog.JEUNO, invaderXim.quest.id.jeuno.THE_LOST_CARDIAN) == invaderXim.questStatus.QUEST_AVAILABLE then
        if player:getCharVar('theLostCardianVar') == 0 then
            player:startEvent(29) -- First dialog for "The lost cardian" quest
        else
            player:startEvent(66)
        end
    elseif theKindCardian == invaderXim.questStatus.QUEST_ACCEPTED then
