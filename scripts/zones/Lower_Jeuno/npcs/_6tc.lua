...
entity.onTrade = function(player, npc, trade)
    if player:getQuestStatus(invaderXim.questLog.JEUNO, invaderXim.quest.id.jeuno.BEAT_AROUND_THE_BUSHIN) == invaderXim.questStatus.QUEST_ACCEPTED then
        if
            trade:hasItemQty(invaderXim.item.WYRM_BEARD, 1) and
        ...
        elseif
            trade:hasItemQty(invaderXim.item.BEHEMOTH_TONGUE, 1) and
            trade:getItemCount() == 1 and
        ...
        elseif
            trade:hasItemQty(invaderXim.item.ADAMANTOISE_EGG, 1) and
            trade:getItemCount() == 1 and
        ...
        elseif
            trade:hasItemQty(invaderXim.item.BROWN_BELT, 1) and
            trade:getItemCount() == 1 and
...
entity.onTrigger = function(player, npc)
    ...
    elseif player:hasKeyItem(invaderXim.ki.TENSHODO_MEMBERS_CARD) then
...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 155 then
        player:addQuest(invaderXim.questLog.JEUNO, invaderXim.quest.id.jeuno.BEAT_AROUND_THE_BUSHIN)
    ...
    elseif csid == 159 then
        if player:getFreeSlotsCount() == 0 then
            player:messageSpecial(ID.text.ITEM_CANNOT_BE_OBTAINED, invaderXim.item.BLACK_BELT)
        else
            player:addTitle(invaderXim.title.BLACK_BELT)
            player:addItem(invaderXim.item.BLACK_BELT)
            player:messageSpecial(ID.text.ITEM_OBTAINED, invaderXim.item.BLACK_BELT)
            player:setCharVar('BeatAroundTheBushin', 0)
            player:addFame(invaderXim.fameArea.NORG, 125)
            player:tradeComplete()
            player:completeQuest(invaderXim.questLog.JEUNO, invaderXim.quest.id.jeuno.BEAT_AROUND_THE_BUSHIN)
        end
    end
end
