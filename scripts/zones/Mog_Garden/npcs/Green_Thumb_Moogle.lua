...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 1016 and option == 0xFFF00FF then -- Show the Mog House menu
        ...
        local lockerLease = invaderXim.moghouse.getMogLockerExpiryTimestamp(player)
        if lockerLease ~= nil then
            if lockerLease == -1 then -- Lease expired..
                player:messageSpecial(ID.text.MOGLOCKER_MESSAGE_OFFSET + 2, invaderXim.item.IMPERIAL_BRONZE_PIECE)
            else
