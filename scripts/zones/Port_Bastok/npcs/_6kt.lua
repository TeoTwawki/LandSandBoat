...
--npc is the moghouse-side door to the bridge
    ...
    if triggerID == 0 or triggerID == 2 then
        ...
        bridge:updateToEntireZone(invaderXim.status.NORMAL, invaderXim.animation.CLOSE_DOOR, true)
        upperDoor:setAnimation(invaderXim.animation.CLOSE_DOOR)
        npc:setAnimation(invaderXim.animation.CLOSE_DOOR)

    elseif triggerID == 1 or triggerID == 3 then
        --drawbridge needs to close
        bridge:updateToEntireZone(invaderXim.status.NORMAL, invaderXim.animation.OPEN_DOOR, true)

    elseif triggerID == 4 or triggerID == 5 then
        --drawbridge has finished closing, reopen entry to walkers
        upperDoor:setAnimation(invaderXim.animation.OPEN_DOOR)
