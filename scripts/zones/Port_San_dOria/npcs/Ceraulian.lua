...
entity.onTrade = function(player, npc, trade)
    if
        player:getQuestStatus(invaderXim.questLog.SANDORIA, invaderXim.quest.id.sandoria.CHASING_QUOTAS) == invaderXim.questStatus.QUEST_ACCEPTED and
        player:getCharVar('ChasingQuotas_Progress') == 0 and
        trade:getItemCount() == 1 and
        trade:hasItemQty(invaderXim.item.GOLD_HAIRPIN, 1) and
        trade:getGil() == 0
...
entity.onTrigger = function(player, npc)
    local craftsmansStatus = player:getQuestStatus(invaderXim.questLog.SANDORIA, invaderXim.quest.id.sandoria.A_CRAFTSMANS_WORK)
    local quotasStatus     = player:getQuestStatus(invaderXim.questLog.SANDORIA, invaderXim.quest.id.sandoria.CHASING_QUOTAS)
    local quotasProgress   = player:getCharVar('ChasingQuotas_Progress')
    local quotasNo         = player:getCharVar('ChasingQuotas_No')
    local stalkerStatus    = player:getQuestStatus(invaderXim.questLog.SANDORIA, invaderXim.quest.id.sandoria.KNIGHT_STALKER)
    ...
    if
        ...
        player:getQuestStatus(invaderXim.questLog.SANDORIA, invaderXim.quest.id.sandoria.THE_HOLY_CREST) == invaderXim.questStatus.QUEST_AVAILABLE
    then
        player:startEvent(24)

    -- Chasing Quotas (DRG AF2)
    elseif
        craftsmansStatus == invaderXim.questStatus.QUEST_COMPLETED and
        quotasStatus == invaderXim.questStatus.QUEST_AVAILABLE and
    ...
    elseif quotasStatus == invaderXim.questStatus.QUEST_ACCEPTED and quotasProgress == 0 then
    ...
    elseif
        quotasStatus == invaderXim.questStatus.QUEST_COMPLETED and
        stalkerStatus == invaderXim.questStatus.QUEST_AVAILABLE
    then
        player:startEvent(16) -- Fluff text until DRG AF3

    -- Knight Stalker (DRG AF3)
    elseif stalkerStatus == invaderXim.questStatus.QUEST_ACCEPTED and stalkerProgress == 0 then
        player:startEvent(19) -- Fetch the last Dragoon's helmet
    elseif stalkerProgress == 1 then
        if not player:hasKeyItem(invaderXim.ki.CHALLENGE_TO_THE_ROYAL_KNIGHTS) then
    ...
    elseif stalkerStatus == invaderXim.questStatus.QUEST_COMPLETED then
...
entity.onEventFinish = function(player, csid, option, npc)
    ...
    elseif csid == 18 then
        if option == 0 then
            ...
            player:addQuest(invaderXim.questLog.SANDORIA, invaderXim.quest.id.sandoria.CHASING_QUOTAS)
        end
    ...
    elseif csid == 15 then
        if player:getFreeSlotsCount() < 1 then
            player:messageSpecial(ID.text.ITEM_CANNOT_BE_OBTAINED, invaderXim.item.DRACHEN_BRAIS)
        else
            player:delKeyItem(invaderXim.ki.RANCHURIOMES_LEGACY)
            player:addItem(invaderXim.item.DRACHEN_BRAIS)
            player:messageSpecial(ID.text.ITEM_OBTAINED, invaderXim.item.DRACHEN_BRAIS) -- Drachen Brais
            player:addFame(invaderXim.fameArea.SANDORIA, 40)
            player:completeQuest(invaderXim.questLog.SANDORIA, invaderXim.quest.id.sandoria.CHASING_QUOTAS)
