...
entity.onTrigger = function(player, npc)
    ...
    if
        player:hasKeyItem(invaderXim.ki.MERTAIRES_BRACELET) and
    ...
    elseif player:hasKeyItem(invaderXim.ki.MERTAIRES_BRACELET) and trosKilled == 1 then
...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 8 then
        if npcUtil.completeQuest(player, invaderXim.questLog.JEUNO, invaderXim.quest.id.jeuno.PAINFUL_MEMORY, { item = 16766 }) then
            player:delKeyItem(invaderXim.ki.MERTAIRES_BRACELET)
