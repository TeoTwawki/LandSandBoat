...
entity.onTrade = function(player, npc, trade)
    ...
    local flyerForRegine = player:getQuestStatus(invaderXim.questLog.SANDORIA, invaderXim.quest.id.sandoria.FLYERS_FOR_REGINE)

    if flyerForRegine == 1 then
        if npcUtil.tradeHasExactly(trade, invaderXim.item.MAGICMART_FLYER) then
...
entity.onTrigger = function(player, npc)
    local guildSkillId = invaderXim.skill.LEATHERCRAFT
