...
entity.onTrigger = function(player, npc)
    if
        player:getQuestStatus(invaderXim.questLog.SANDORIA, invaderXim.quest.id.sandoria.UNDER_OATH) == invaderXim.questStatus.QUEST_ACCEPTED and
    ...
    elseif
        ...
        player:hasKeyItem(invaderXim.ki.MIQUES_PAINTBRUSH)
    then
        player:startEvent(5) --Under Oath - upset about the paintbrush
    elseif
        player:getCharVar('UnderOathCS') == 4 and
        player:hasKeyItem(invaderXim.ki.STRANGE_SHEET_OF_PAPER)
    then
...
entity.onEventFinish = function(player, csid, option, npc)
    ...
    elseif csid == 5 then
        ...
        player:delKeyItem(invaderXim.ki.MIQUES_PAINTBRUSH)
    elseif csid == 2 then
        player:setCharVar('UnderOathCS', 0)
        player:delKeyItem(invaderXim.ki.KNIGHTS_CONFESSION)
    end
end
