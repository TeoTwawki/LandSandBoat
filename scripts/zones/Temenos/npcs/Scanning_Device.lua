...
entity.onTrade = function(player, npc, trade)
    ...
    elseif npcUtil.tradeHas(trade, invaderXim.item.IVORY_CHIP) then
        player:startEvent(121, 33)
    elseif npcUtil.tradeHas(trade, invaderXim.item.SCARLET_CHIP) then
        player:startEvent(121, 65)
    elseif npcUtil.tradeHas(trade, invaderXim.item.EMERALD_CHIP) then
