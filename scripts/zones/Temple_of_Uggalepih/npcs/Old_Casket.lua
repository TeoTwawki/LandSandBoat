...
entity.onTrigger = function(player, npc)
    if player:hasKeyItem(invaderXim.ki.OLD_RUSTY_KEY) then
        player:startEvent(64, invaderXim.ki.OLD_RUSTY_KEY)
...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 64 and option == 1 then
        player:delKeyItem(invaderXim.ki.OLD_RUSTY_KEY)
        npcUtil.giveKeyItem(player, invaderXim.ki.PAINTBRUSH_OF_SOULS)
    end
end
