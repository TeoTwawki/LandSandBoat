...
entity.onTrigger = function(player, npc)
    ...
    if
        ...
        pet and
        pet:getPetID() == invaderXim.petId.WYVERN and
...
entity.onEventFinish = function(player, csid, option, npc)
    if
        csid == 67 and
        npcUtil.completeQuest(player, invaderXim.questLog.SANDORIA, invaderXim.quest.id.sandoria.KNIGHT_STALKER, { item = 12519, fame = 60, title = invaderXim.title.PARAGON_OF_DRAGOON_EXCELLENCE, var = { 'KnightStalker_Kill', 'KnightStalker_Progress' } })
    then
        player:delKeyItem(invaderXim.ki.CHALLENGE_TO_THE_ROYAL_KNIGHTS)
