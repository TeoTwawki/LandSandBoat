...
entity.onTrigger = function(player, npc)
    ...
    if player:hasKeyItem(invaderXim.ki.MYSTIC_ICE) then
        if z > -200 and z < -150 then                -- southern Fissure (J-9)
            player:startEvent(2, invaderXim.ki.MYSTIC_ICE)
        elseif z > 200 and z < 250 then            -- middle Fissure (K-7)
            player:startEvent(3, invaderXim.ki.MYSTIC_ICE)
        elseif z > 450 then                        -- northern Fissure (I-6)
            player:startEvent(4, invaderXim.ki.MYSTIC_ICE)
        end
    else
...
entity.onEventFinish = function(player, csid, option, npc)
    if (csid == 2 or csid == 3 or csid == 4) and option == 2 then
        player:delKeyItem(invaderXim.ki.MYSTIC_ICE)
    end
end
