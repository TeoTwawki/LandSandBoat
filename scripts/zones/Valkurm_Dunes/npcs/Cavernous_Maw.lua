...
entity.onTrigger = function(player, npc)
    if invaderXim.settings.main.ENABLE_ABYSSEA == 1 and player:getMainLvl() >= 30 then
        local hasStone = invaderXim.abyssea.getHeldTraverserStones(player)
        ...
        if
            ...
            player:getQuestStatus(invaderXim.questLog.ABYSSEA, invaderXim.quest.id.abyssea.A_DELECTABLE_DEMON) == invaderXim.questStatus.QUEST_AVAILABLE
        then
...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 56 then
        player:addQuest(invaderXim.questLog.ABYSSEA, invaderXim.quest.id.abyssea.A_DELECTABLE_DEMON)
    elseif csid == 57 then
        -- Killed Cirein-croin
