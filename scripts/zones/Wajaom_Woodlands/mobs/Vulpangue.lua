...
entity.onMobSpawn = function(mob)
    ...
    mob:addMod((invaderXim.mod.FIRE_ABSORB + VanadielDayElement() - 1), 100)
    mob:addMod(invaderXim.mod.WIND_ABSORB, 100)
...
entity.onMobFight = function(mob, target)
    ...
    if mob:getHPP() <= defUpHPP then
        if mob:getHPP() > 10 then
            mob:addMod(invaderXim.mod.ACC, 10)
            mob:addMod(invaderXim.mod.ATT, 10)
