...
entity.onTrade = function(player, npc, trade)
    ...
    then
        if trade:hasItemQty(invaderXim.item.HYDROGAUGE, 1) and trade:getItemCount() == 1 then -- Trade Hydrogauge
            player:messageSpecial(ID.text.PLACE_HYDROGAUGE, invaderXim.item.HYDROGAUGE) -- You set the <item> in the trench.
...
entity.onTrigger = function(player, npc)
    ...
    then
        if player:getCharVar('Leypoint_waitJTime') <= os.time() then
            ...
            player:messageSpecial(ID.text.ENIGMATIC_LIGHT, invaderXim.item.HYDROGAUGE)    -- play cs for not waiting long enough
        end
    end
end
