...
entity.onTrade = function(player, npc, trade)
    if
        player:getQuestStatus(invaderXim.questLog.WINDURST, invaderXim.quest.id.windurst.TO_CATCH_A_FALLING_STAR) == invaderXim.questStatus.QUEST_ACCEPTED and
        VanadielHour() <= 3
    then
        if
            npcUtil.tradeHas(trade, invaderXim.item.HANDFUL_OF_PUGIL_SCALES) and
        ...
        then
            ...
            if npcUtil.giveItem(player, invaderXim.item.STARFALL_TEAR) then
                player:confirmTrade()
