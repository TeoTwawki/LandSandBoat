    ...
    then
        if not maskBit3 then
            ...
            if not maskBit2 or not maskBit1 then
                player:messageSpecial(ID.text.DOOR_OFFSET + 1, invaderXim.ki.ZONPA_ZIPPAS_ALL_PURPOSE_PUTTY)
            else
                player:messageSpecial(ID.text.DOOR_OFFSET + 4, invaderXim.ki.ZONPA_ZIPPAS_ALL_PURPOSE_PUTTY)
                player:delKeyItem(invaderXim.ki.ZONPA_ZIPPAS_ALL_PURPOSE_PUTTY)
            end
        else
            player:messageSpecial(ID.text.DOOR_OFFSET + 2, invaderXim.ki.ZONPA_ZIPPAS_ALL_PURPOSE_PUTTY)
        end
    elseif snakeOnThePlains == invaderXim.questStatus.QUEST_COMPLETED then
        player:messageSpecial(ID.text.DOOR_OFFSET + 2, invaderXim.ki.ZONPA_ZIPPAS_ALL_PURPOSE_PUTTY)
    else
