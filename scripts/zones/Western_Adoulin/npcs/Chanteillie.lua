...
entity.onTrade = function(player, npc, trade)
    local dngitl = player:getQuestStatus(invaderXim.questLog.ADOULIN, invaderXim.quest.id.adoulin.DO_NOT_GO_INTO_THE_LIGHT)
    local vvc = player:getQuestStatus(invaderXim.questLog.ADOULIN, invaderXim.quest.id.adoulin.VEGETABLE_VEGETABLE_CRISIS)

    -- DO NOT GO INTO THE LIGHT (Urunday Lumber, Damascus Ingot, Fire Crystal)
    if
        dngitl == invaderXim.questStatus.QUEST_ACCEPTED and
    ...
    elseif
        vvc == invaderXim.questStatus.QUEST_ACCEPTED and
...
entity.onTrigger = function(player, npc)
    ...
    if
        dngitl == invaderXim.questStatus.QUEST_ACCEPTED and
        player:hasKeyItem(invaderXim.ki.INVENTORS_COALITION_PICKAXE)
    then
        player:startEvent(5077)

    -- VEGETABLE VEGETABLE CRISIS
    elseif vvc == invaderXim.questStatus.QUEST_ACCEPTED and player:getCharVar('VVC_Status') == 1 then
...
entity.onEventFinish = function(player, csid, option, npc)
    ...
    if csid == 5076 then
        player:confirmTrade()
        npcUtil.giveKeyItem(player, invaderXim.ki.INVENTORS_COALITION_PICKAXE)
