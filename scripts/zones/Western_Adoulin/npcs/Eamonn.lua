...
entity.onTrigger = function(player, npc)
    local scaredyCats = player:getQuestStatus(invaderXim.questLog.ADOULIN, invaderXim.quest.id.adoulin.SCAREDYCATS)
    local scaredyCatsStatus = player:getCharVar('Scaredycats_Status')

    if scaredyCatsStatus < 1 and scaredyCats == invaderXim.questStatus.QUEST_AVAILABLE then
    ...
    elseif scaredyCatsStatus == 1 then
        if scaredyCats == invaderXim.questStatus.QUEST_ACCEPTED then
...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 5024 and option == 1 then
        ...
        player:addQuest(invaderXim.questLog.ADOULIN, invaderXim.quest.id.adoulin.SCAREDYCATS)
    end
end
