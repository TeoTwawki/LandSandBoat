...
entity.onTrigger = function(player, npc)
    ...
    if orderUp == invaderXim.questStatus.QUEST_ACCEPTED and not orderGrevan then
        -- Progresses Quest: 'Order Up'
        player:startEvent(69)
    else
        if player:getQuestStatus(invaderXim.questLog.ADOULIN, invaderXim.quest.id.adoulin.HYPOCRITICAL_OATH) == invaderXim.questStatus.QUEST_COMPLETED then
            if player:getQuestStatus(invaderXim.questLog.ADOULIN, invaderXim.quest.id.adoulin.NOTSOCLEAN_BILL) == invaderXim.questStatus.QUEST_COMPLETED then
