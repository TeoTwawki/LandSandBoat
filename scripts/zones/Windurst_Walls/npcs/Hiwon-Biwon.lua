...
entity.onTrigger = function(player, npc)
    ...
    local cursesFoiledAgain1 = player:getQuestStatus(invaderXim.questLog.WINDURST, invaderXim.quest.id.windurst.CURSES_FOILED_AGAIN_1)

    -- Making Headlines
    if makingHeadlines == invaderXim.questStatus.QUEST_ACCEPTED then
        ...
        if not utils.mask.getBit(prog, 2) then
            if cursesFoiledAgain1 == invaderXim.questStatus.QUEST_ACCEPTED then
...
entity.onEventFinish = function(player, csid, option, npc)
    ...
    if csid == 281 or csid == 283 or csid == 284 then
        npcUtil.giveKeyItem(player, invaderXim.ki.WINDURST_WALLS_SCOOP)
