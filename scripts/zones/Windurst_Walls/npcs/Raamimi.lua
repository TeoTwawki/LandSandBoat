...
entity.onTrigger = function(player, npc)
    ...
    if toBeeOrNotStatus == 10 and toBee == invaderXim.questStatus.QUEST_AVAILABLE then
    ...
    elseif toBee == invaderXim.questStatus.QUEST_COMPLETED and toBeeOrNotStatus == 5 then
        player:startEvent(80) -- Quest Finish - Gives Mulsum
    elseif
        toBee == invaderXim.questStatus.QUEST_COMPLETED and
...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 67 then
        if player:getFreeSlotsCount() == 0 then
            player:messageSpecial(ID.text.ITEM_CANNOT_BE_OBTAINED, invaderXim.item.POT_OF_HONEY) -- Cannot give Honey because player Inventory is full
        else
            player:addQuest(invaderXim.questLog.WINDURST, invaderXim.quest.id.windurst.TO_BEE_OR_NOT_TO_BEE)
            player:addItem(invaderXim.item.POT_OF_HONEY)
            player:messageSpecial(ID.text.ITEM_OBTAINED, invaderXim.item.POT_OF_HONEY) -- Gives player Honey x1
        end
    elseif csid == 80 then -- After Honey#5: ToBee quest Finish (tooth hurts from all the Honey)
        if player:getFreeSlotsCount() == 0 then
            ...
            player:addItem(invaderXim.item.BOTTLE_OF_MULSUM, 3) -- Mulsum x3
            player:messageSpecial(ID.text.ITEMS_OBTAINED, invaderXim.item.BOTTLE_OF_MULSUM, 3)
