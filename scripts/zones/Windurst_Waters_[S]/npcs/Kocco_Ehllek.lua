...
entity.onTrigger = function(player, npc)
    local redeemingRocks = player:getQuestStatus(invaderXim.questLog.CRYSTAL_WAR, invaderXim.quest.id.crystalWar.REDEEMING_ROCKS)
    local rocksProg = player:getCharVar('RedeemingRocksProg')

    if redeemingRocks == invaderXim.questStatus.QUEST_AVAILABLE then
        player:startEvent(108) -- Start quest "Redeeming Rocks"
    elseif redeemingRocks == invaderXim.questStatus.QUEST_ACCEPTED and rocksProg == 2 then
        player:startEvent(109) -- 3rd CS quest "Redeeming Rocks"
    elseif redeemingRocks == invaderXim.questStatus.QUEST_ACCEPTED and rocksProg == 4 then
        player:startEvent(110) -- 4th CS quest "Redeeming Rocks"
    elseif
        redeemingRocks == invaderXim.questStatus.QUEST_ACCEPTED and
...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 108 then -- Finish 'Redeeming Rocks' opening CS
        player:addQuest(invaderXim.questLog.CRYSTAL_WAR, invaderXim.quest.id.crystalWar.REDEEMING_ROCKS)
    ...
    elseif csid == 111 then -- Finish 'Redeeming Rocks' quest
        npcUtil.completeQuest(player, invaderXim.questLog.CRYSTAL_WAR, invaderXim.quest.id.crystalWar.REDEEMING_ROCKS, {
            item = 15998,
            var = { 'RedeemingDay' }
        })
    end
